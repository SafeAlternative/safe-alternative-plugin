jQuery(function ($) {
    $('<option>').val('generateAWB_Memex').text('Genereaza AWB Memex').appendTo("select[name='action'], select[name='action2']");

    $('#doaction, #doaction2').click(function (e) {
        let select = $(this).siblings('select');

        if (select.val() === 'generateAWB_Memex') {
            e.preventDefault();
            var urls = [];
            let checked_inputs = $('input[name="post[]"]:checked');

            checked_inputs.each(function (key, element) {
                let generateAWBbtn = $(element).parents('th').siblings('td.Memex_AWB');
