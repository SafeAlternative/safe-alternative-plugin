jQuery(function ($) {
    $('<option>').val('generateAWB_Express').text('Genereaza AWB Express').appendTo("select[name='action'], select[name='action2']");

    $('#doaction, #doaction2').click(function (e) {
        let select = $(this).siblings('select');

        if (select.val() === 'generateAWB_Express') {
            e.preventDefault();
            var urls = [];
            let checked_inputs = $('input[name="post[]"]:checked');

            checked_inputs.each(function (key, element) {
                let generateAWBbtn = $(element).parents('th').siblings('td.Express_AWB');
