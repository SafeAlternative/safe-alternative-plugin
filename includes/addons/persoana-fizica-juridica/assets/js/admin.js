(function ($) {
	...
	$(document).ready(function () {
		var wrapper = $('#order_data .order_data_column_container .order_data_column:nth-child(2) div.edit_address'),
			tip_facturare_el = wrapper.find('#_billing_safealternative_pf_pj__type'),
			tip_facturare = '',
			el_class = '';
		...
		if (tip_facturare_el.length > 0) {
			...
			wrapper.find('.safealternative_pf_pj_option_field').hide();
			wrapper.find('.safealternative_pf_pj_option_field').filter(el_class).show();
