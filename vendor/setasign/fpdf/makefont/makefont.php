...
function LoadMap($enc)
{
    ...
    $a = \file($file);
    if (empty($a)) {
        \SafeAlternative\Error('Encoding not found: ' . $enc);
    }
...
function GetInfoFromTrueType($file, $embed, $subset, $map)
{
    ...
    try {
        ...
        \SafeAlternative\Error($e->getMessage());
    }
    if ($embed) {
        if (!$ttf->embeddable) {
            \SafeAlternative\Error('Font license does not allow embedding');
        }
        if ($subset) {
            $chars = array();
            foreach ($map as $v) {
...
function GetInfoFromType1($file, $embed, $map)
{
    ...
    if ($embed) {
        $f = \fopen($file, 'rb');
        if (!$f) {
            \SafeAlternative\Error('Can\'t open font file');
        }
        // Read first segment
        $a = \unpack('Cmarker/Ctype/Vsize', \fread($f, 6));
        if ($a['marker'] != 128) {
            \SafeAlternative\Error('Font file is not a valid binary Type1');
        }
        $size1 = $a['size'];
        $data = \fread($f, $size1);
        // Read second segment
    ...
    if (!\file_exists($afm)) {
        \SafeAlternative\Error('AFM font file not found: ' . $afm);
    }
    $a = \file($afm);
    if (empty($a)) {
        \SafeAlternative\Error('AFM file empty or not readable');
    }
    foreach ($a as $line) {
        $e = \explode(' ', \rtrim($line));
        if (\count($e) < 2) {
    ...
    if (!isset($info['FontName'])) {
        \SafeAlternative\Error('FontName missing in AFM file');
    }
...
function SaveToFile($file, $s, $mode)
{
    $f = \fopen($file, 'w' . $mode);
    if (!$f) {
        \SafeAlternative\Error('Can\'t write to file ' . $file);
    }
    \fwrite($f, $s);
    \fclose($f);
}
...
function MakeFont($fontfile, $enc = 'cp1252', $embed = \true, $subset = \true)
{
    ...
    if (!\file_exists($fontfile)) {
        \SafeAlternative\Error('Font file not found: ' . $fontfile);
    }
    ...
    if ($ext == 'ttf' || $ext == 'otf') {
        ...
        \SafeAlternative\Error('Unrecognized font file extension: ' . $ext);
    }
    $map = LoadMap($enc);
    if ($type == 'TrueType') {
