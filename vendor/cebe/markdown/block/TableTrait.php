 ...
 */
namespace SafeAlternative\cebe\markdown\block;
...
trait TableTrait
{
    ...
    {
        return \strpos($line, '|') !== \false && isset($lines[$current + 1]) && \preg_match('~^\\s*\\|?(\\s*:?-[\\-\\s]*:?\\s*\\|?)*\\s*$~', $lines[$current + 1]) && \strpos($lines[$current + 1], '|') !== \false && isset($lines[$current + 2]) && \trim($lines[$current + 1]) !== '';
    }
    /**
    ...
    protected function renderTable($block)
    {
        ...
        foreach ($block['rows'] as $row) {
            ...
            foreach ($row as $c => $cell) {
                ...
                $tds .= "<{$cellTag}{$align}>" . \trim($this->renderAbsy($cell)) . "</{$cellTag}>";
            }
            if ($first) {
                $head .= "<tr>{$tds}</tr>\n";
            } else {
